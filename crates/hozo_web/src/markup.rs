...
fn element_shape_inner(node: &Node, diagnostics: &mut Vec<Diagnostic>) -> (&'static str, Vec<(&'static str, AttrValue)>) {
    match node.primitive {
        ...
        Primitive::Dl if node.props.on_layout.is_some() => ("Dl", Vec::new()),
        Primitive::Dl => ("dl", Vec::new()),
        Primitive::Dt if node.props.on_layout.is_some() => ("Dt", Vec::new()),
        Primitive::Dt => ("dt", Vec::new()),
        Primitive::Dd if node.props.on_layout.is_some() => ("Dd", Vec::new()),
        Primitive::Dd => ("dd", Vec::new()),

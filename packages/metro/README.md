...
Without it, an app is type-checked against the **Web** declarations: a native-only export such as `HozoText` from `@hozo/core/generated/text` reads as missing, and a component whose two halves differ is checked against the wrong half. It still bundles and runs correctly — this is the type layer only — which is why it goes unnoticed.

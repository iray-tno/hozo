...
test('withHozo preserves Metro settings and records the existing transformer', async () => {
  ...
  try {
    ...
    const projectRuntime = result.resolver.resolveRequest(
      {
        originModulePath: path.join(root, 'src', 'App.tsx'),
        resolveRequest() {
    ...
    assert.deepEqual(projectRuntime, {
      type: 'sourceFile',
      filePath: path.join(root, 'node_modules', '.hozo', 'candidates.native.js'),
    })
  } finally {
...
test('withHozo accepts a promised config and an explicit project root', async () => {
  ...
  try {
    ...
    assert.match(
      readFileSync(path.join(root, 'node_modules', '.hozo', 'candidates.native.js'), 'utf8'),
      /export const hozoPreflight = false/,
    )
  } finally {
    rmSync(root, { recursive: true, force: true })
  }

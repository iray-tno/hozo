...
| textures and UV sampling | partial | A clamped sRGB MeshBasicMaterial colour map with complete UVs uses affine Canvas sampling; repeat seams, modulation, mipmaps, and other texture roles stay diagnostic. | [test](src/project.test.ts) `MeshBasicMaterial map and UVs become a portable textured triangle`<br>[test](src/project.test.ts) `mesh clipping interpolates texture coordinates at generated edges`<br>[test](src/project.test.ts) `unsupported MeshBasicMaterial features emit diagnostics`<br>[test](src/project.test.ts) `textured points are omitted with a diagnostic` |

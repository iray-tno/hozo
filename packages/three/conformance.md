...
| vertex colours | partial | Per-point RGB and solid-line gradients work; mesh colours and dashed-line colour gradients remain diagnosed. | [test](src/project.test.ts) `PointsMaterial multiplies per-point RGB colours`<br>[test](src/project.test.ts) `LineBasicMaterial projects clipped RGB vertex colours as a portable gradient`<br>[test](src/project.test.ts) `unsupported MeshBasicMaterial features emit diagnostics` |

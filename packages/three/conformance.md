...
| textures and UV sampling | partial | Clamped sRGB MeshBasicMaterial and SpriteMaterial colour maps use affine Canvas sampling; repeat seams, modulation, mipmaps, and other texture roles stay diagnostic. | [test](src/project.test.ts) `MeshBasicMaterial map and UVs become a portable textured triangle`<br>[test](src/project.test.ts) `mesh clipping interpolates texture coordinates at generated edges`<br>[test](src/project.test.ts) `SpriteMaterial map preserves billboard UVs through clipping`<br>[test](src/project.test.ts) `unsupported MeshBasicMaterial features emit diagnostics`<br>[test](src/project.test.ts) `textured points are omitted with a diagnostic` |

...
全 CSS 名で 479/522 (91.8%)、両 platform に名前が存在するか、同じ typed IR へ
正確に展開できる集合で 134/134 (100%)、contextual runtime 集合で 21/21
(100%)、Web-only 集合で 324/366 (88.5%)。残りは optional adapter 候補 1、
...
現在の実行可能scorecardでは、代表value 316/316 (100%)、一般的なauthoring
...
generated `content` は `normal` / `none`、静的な引用文字列、quote制御keywordを
exact Web-onlyとして扱う。`attr()` / `counter()`などのfunction構文は任意CSSを
過剰受理せずに文法を広げるまで公式StyleXへ残す。
`caret` shorthandはWebで`caret-color` / `caret-shape`の独立slotへ展開し、longhandとの
property-specificityを保持する。React Nativeはshape成分を表現できないため、shorthand
全体を明示的にWeb-onlyとしてrefuseする。
...
同時再生するanimation-name list、より広いeasing構文は公式StyleX側のresidualに残す。
単一の`animationRange` / `scrollTimeline` / `viewTimeline` shorthandはWebで独立した
longhand slotへ展開し、listやdynamic timeline functionはresidualに残す。
...
resetも失わない。source-firstの`maskBorder` shorthandもslice/width/outset/repeat/modeを
検証し、CSS resetを保持する。複数layer、より広いimage function、variable、異なる
component順は近似せず公式StyleXへ残す。

anchor positioningではStyleX公開型の`positionArea` keywordと、静的な
...
旧CSS box moduleの `boxAlign` / `boxDirection` / `boxFlex` / `boxFlexGroup` /
`boxLines` / `boxOrdinalGroup` / `boxOrient` / `boxSuppress` は、公式StyleX 0.19が
実際に生成する値だけをexact Web-onlyとして扱う。numeric値に `px` を付ける本家の
歴史的な出力も差分を作らず保持し、Nativeでは明示的にrefuseする。
...
shape、position、angle、lengthを対象にする。単一pathを先頭に置く一般的な`offset`
shorthandもCSS initial値を含む5つの独立longhand slotへ展開する。position/anchorを含む
slash形式、方向相対float、calc、新しいshape構文は公式StyleXのresidualに残す。
...
対象にする。source-firstの実用的な`borderImage` shorthandも構成要素を検証してCSS
shorthandのまま保持し、resetを失わない。より広いimage function、calc、variable、
異なるcomponent順は公式StyleXのresidualに残す。Nativeでは明示的にrefuseする。
...
対応外の Grid shorthand と個別 line の衝突は現在の Grid IR では分解できない。この二種類は
